...
def parse_option() :
    ...
    # pretrained model
    parser.add_argument('--pretrained_model',type = list, default =['random', 'shape', 'texture', 'ours'])
...
def get_pretrained_model(opt, source_type, n_cls) :
...
def main() :
    ...
    for i in range(len(opt.n_style)) : 
        ...
        # get datasets
        train_path = os.path.join('../dataset/cifar100', 'train') 
        test_path = os.path.join('../dataset/cifar100', 'test')
        ...
        for source_type in opt.pretrained_model : 
            # model
            model = resnet50(pretrained = False)
            model = get_pretrained_model(opt, source_type, n_cls = 10)
            ...
            for epoch in range(1, opt.epochs + 1):
                ...
                if best_acc < test_acc :
                    best_acc = test_acc
                    print('==> Saving...')
                    save_file = os.path.join(f'./checkpoint_0304/svhn_{source_type}_{opt.n_style[i]}_best.pth'.format(epoch=epoch))
